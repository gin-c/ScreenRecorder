...
class AudioPlaybackRecorder implements Encoder {
    ...
    AudioPlaybackRecorder(AudioEncodeConfig config, AudioPlaybackCaptureConfiguration audioPlaybackCaptureConfiguration) {
        mEncoder = new AudioEncoder(config);
        mSampleRate = config.sampleRate;
        mChannelsSampleRate = mSampleRate * config.channelCount;
        mAudioPlaybackCaptureConfiguration = audioPlaybackCaptureConfiguration;
    ...
    private class RecordHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case MSG_PREPARE:
                    AudioRecord r = createAudioRecord(mSampleRate, mChannelConfig, mFormat, mAudioPlaybackCaptureConfiguration);
                    if (r == null) {
    ...
    @TargetApi(29)
    private static AudioRecord createAudioRecord(int sampleRateInHz, int channelConfig, int encoding, AudioPlaybackCaptureConfiguration audioPlaybackCaptureConfiguration) {
        int minBytes = AudioRecord.getMinBufferSize(sampleRateInHz, channelConfig, encoding);
        if (minBytes <= 0) {
            Log.e(TAG, String.format(Locale.US, "Bad arguments: getMinBufferSize(%d, %d, %d)",
                    sampleRateInHz, channelConfig, encoding));
            return null;
        }
        AudioFormat audioFormat = new AudioFormat.Builder().setEncoding(encoding).setSampleRate(sampleRateInHz).setChannelMask(channelConfig).build();


        @SuppressLint("MissingPermission") AudioRecord record = new AudioRecord.Builder().setAudioFormat(audioFormat).setBufferSizeInBytes(minBytes * 2).setAudioPlaybackCaptureConfig(audioPlaybackCaptureConfiguration).build();
